...
def get_regularizer(model, old_state_dict, device, cfg, old_state):
    resume = False
    ...
    if name is None:
        ...
        return EWC(model, old_state_dict, device, fisher=fisher,
                   alpha=cfg.reg_alpha,
                   normalize= not cfg.reg_no_normalize)
    elif name == 'pi':
        score = old_state["score"] if resume else None
        return PI(model, old_state_dict, device, score=score,
                  normalize=not cfg.reg_no_normalize)
    elif name == 'rw':
        score = old_state["score"] if resume else None
        fisher = old_state["fisher"] if resume else None
        return RW(model, old_state_dict, device, score=score, fisher=fisher,
                  alpha=cfg.reg_alpha, iterations=cfg.reg_iterations,
                  normalize=not cfg.reg_no_normalize)
    else:
...
class EWC(Regularizer):
    ...
    def __init__(self, model, old_model, device, fisher=None, alpha=0.9, normalize=True):
        ...
        if old_model is not None:
            self.old_state_dict_no_module = old_model.state_dict()
            self.old_state_dict = {f"module.{key}": value for key, value in self.old_state_dict_no_module.items()}
            self.penalize = True
        else:
            self.penalize = False
    ...
    def penalty(self):
        if not self.penalize:
            ...
            loss = 0.
            for n, p in self.model.named_parameters():
                if n in self.old_state_dict and p.requires_grad:
                    self.old_state_dict[n] = self.old_state_dict[n].to(p.device)
                    loss += (self.fisher_old[n] * (p - self.old_state_dict[n]) ** 2).sum()
            return loss
