...
def _local_pod(x, spp_scales=[1, 2, 4], normalize=False, normalize_per_scale=False):
    ...
    for scale in spp_scales:
        k = w // scale
        # 使用 unfold 提取局部区域
        unfolded = x.unfold(2, k, k).unfold(3, k, k)  # (b, c, scale, scale, k, k)
        unfolded = unfolded.contiguous().view(b, c, -1, k, k)  # (b, c, scale*scale, k, k)

        # 计算水平和垂直池化
        horizontal_pool = unfolded.mean(dim=4).view(b, -1)  # (b, c * scale * scale * k)
        vertical_pool = unfolded.mean(dim=3).view(b, -1)    # (b, c * scale * scale * k)

        if normalize_per_scale:
            horizontal_pool = horizontal_pool / (scale * scale)
            vertical_pool = vertical_pool / (scale * scale)
...
# def _local_pod_masked(x, mask, spp_scales=[1, 2, 4], normalize=False, normalize_per_scale=False):
#     b = x.shape[0]
#     c = x.shape[1]
#     w = x.shape[-1]
#     emb = []
...
def _local_pod_masked(x, mask, spp_scales=[1, 2, 4], normalize=False, normalize_per_scale=False):
    ...
    for scale in spp_scales:
        ...
        # 提取掩码的局部区域
        mask_unfolded = mask.unfold(2, k, k).unfold(3, k, k)  # (b, 1, scale, scale, k, k)
        mask_unfolded = mask_unfolded.contiguous().view(b, -1, k, k)  # (b, scale*scale, k, k)

        # 应用掩码
        unfolded = unfolded * mask_unfolded[:, None, :, :, :]
