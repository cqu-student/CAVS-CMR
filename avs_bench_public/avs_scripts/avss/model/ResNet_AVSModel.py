...
class Pred_endecoder(nn.Module):
    ...
    def forward(self, x, audio_feature, vid_temporal_mask_flag):
        ...
        for layer in self.resnet.layer1:
            attentions.append(layer.attentions)
        for layer in self.resnet.layer2:
            attentions.append(layer.attentions)
        for layer in self.resnet.layer3_1:
            attentions.append(layer.attentions)
        for layer in self.resnet.layer4_1:
            attentions.append(layer.attentions)
        ...
        conv4321 = conv4321 * vid_temporal_mask_flag  #BF × 256 × 112 × 112  #! notice  
        # print(conv4_feat.shape, conv43.shape, conv432.shape, conv4321.shape)
        pred = self.output_conv(conv4321)    #BF × 32 ×224 ×224
        out = []
